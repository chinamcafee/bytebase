...
// DISABLED: All metrics-related constants are unused since metrics collection is disabled.
// The following constants are kept for reference but are not used:
/*
const (
...
func NewReporter(store *store.Store, licenseService *enterprise.LicenseService, profile *config.Profile) *Reporter {
	// Always use mock reporter to prevent any metrics collection
...
// DISABLED: Metrics collection disabled - function returns immediately.
func (m *Reporter) Run(ctx context.Context, wg *sync.WaitGroup) {
	// Metrics collection is disabled to prevent communication with external servers
	// Just mark the goroutine as done and return
	defer wg.Done()

	slog.Debug("Metrics reporter is DISABLED - no metrics will be collected")
	return
...
// DISABLED: Always returns false to prevent metrics collection.
func (m *Reporter) isMetricCollectionEnabled(ctx context.Context) bool {
	// Metrics collection is always disabled to prevent communication with external servers
	slog.Debug("Metrics collection is DISABLED - isMetricCollectionEnabled returning false")
	return false
...
// DISABLED: Workspace identification disabled to prevent communication with external servers.
func (m *Reporter) identify(ctx context.Context) (string, error) {
	// Workspace identification is disabled to prevent communication with external servers
	// Just return workspace ID without sending any data
	workspaceID, err := m.store.GetWorkspaceID(ctx)
	if err != nil {
		return "", err
	}

	slog.Debug("Workspace identification is DISABLED - not sending identification data")
...
// DISABLED: Metrics reporting disabled to prevent communication with external servers.
func (m *Reporter) Report(ctx context.Context, metric *metric.Metric) {
	// Metrics reporting is disabled - simply return without doing anything
	slog.Debug("Metrics reporting is DISABLED - metric will not be reported", slog.String("metric", string(metric.Name)))
	return
